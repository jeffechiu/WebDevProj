...
function search() {
  ...
  var chosenType = [
    "directoryEntry",
    "directoryEntryPeople",
    "directoryEntryNonprofit",
    "directoryEntryBusiness",
  ];

  if (input.value == "") {
    for (var i = 0; i < entities.length; i++) {
      if (
        entities[i].classList[entities[i].classList.length - 1] ==
        "entityActive"
      ) {
        for (var j = 0; j < entries.length; j++) {
          if (
            i == 0 ||
            entries[j].classList[entries[j].classList.length - 1] ==
              chosenType[i]
          ) {
            if (i == 0) {
  ...
  for (var i = 0; i < entities.length; i++) {
    if (
      entities[i].classList[entities[i].classList.length - 1] == "entityActive"
    ) {
      res = i;
    }
  }
...
function search2() {
  ...
  var chosenTypeDict = {
    entityAll: "directoryEntry",
    entityPeople: "directoryEntryPeople",
    entityNonprofits: "directoryEntryNonprofit",
    entityBusinesses: "directoryEntryBusiness",
  };

  if (input.value == "") {
    for (var j = 0; j < entries.length; j++) {
      if (
        entities[0].id.split(" ")[1] == "entityAll" ||
        entries[j].classList[entries[j].classList.length - 1] ==
          chosenTypeDict[entities[0].id.split(" ")[1]]
      ) {
  ...
  for (var i = 0; i < entities.length; i++) {
    if (entities[i].id == id) {
      ...
      for (var j = 0; j < entries.length; j++) {
        if (
          i == 0 ||
          entries[j].classList[entries[j].classList.length - 1] == chosenType[i]
        ) {
          if (i == 0) {
            if (entries[j].classList.contains("de2")) {
              entries[j].style.display = "none";
...
function filterEntities2(id) {
  ...
  var chosenTypeDict = {
    directoryEntry: "entityAll",
    directoryEntryPeople: "entityPeople",
    directoryEntryNonprofit: "entityNonprofits",
    directoryEntryBusiness: "entityBusinesses",
  };
